...
```text
2953c72a10e38d03fa0d99233813d52beed32562b47ed597770efd0ef4a063cc  Pi-Desk-0.2.20-arm64.dmg
92ac6c1a85aeb0023018567538fc0e12097c67317544efce6af3d653115978d2  Pi-Desk-0.2.20-x64.dmg
```

- 完成长对话渐进渲染与历史分页、状态详情、文件引用、原生队列查看与发送模式、未读/后台通知、独立中断回复预览；无新增运行时依赖。
- 依据本机 Pi RPC 源码核对 `steer`、`follow_up`、`clear_queue`、重试事件；不增加逐条队列删除、自己的执行队列或重试器。通知只读取已识别扩展的配置判定重复，不修改扩展或本机配置。
- 恢复缓存与 Pi JSONL 分离，按会话路径散列命名、权限 0600 原子替换、单条文本容量限制；用户消息指纹核对分支，原生记录去重后清理，不从恢复缓存发起模型请求。
- 已通过 TypeScript 类型检查与 Electron/Vite 生产构建；源码提交 `f5aad5e`。双架构安装包通过签名、架构、镜像和打包内容一致性校验。按用户约定未做浏览器交互、手工功能、崩溃/重试复现或验收测试。
- 已通过本地 arm64 安装包正常退出、替换并重新打开，确认本机版本为 0.2.20；旧应用临时副本已删除。
- 双架构 GitHub 发布流程已启动；只有远端资产摘要与本地一致后脚本才会公开版本并清理解包副本，公开状态以 GitHub Releases 为准。

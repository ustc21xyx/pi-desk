...
当前是 **0.2.24 开发预览**，目标是先打通真实本地工作流。代码经过类型检查与构建；依用户要求未进行界面/功能验收；历史会话命名已按用户授权调用指定模型执行。扩展与运行时行为仍需实际使用反馈。

## 已实现的首版范围

- 长对话默认挂载最近 60 条记录，上翻每次展开 40 条，已加载记录不足时向前读取 100 条；回到最新消息缩小挂载范围。历史文件仍有 64 MB 读取上限，按当前分支读取，不改写源文件。
- 点击运行状态查看阶段用时、最近事件和 Pi 原生重试进度；较长时间没有事件时提示等待或手动停止，不自动终止工具或重发请求。
- `@` 文件搜索只索引路径，最多 8 个候选，支持方向键、Enter/Tab、Esc 与中文输入法；空格路径自动引用。通过现有文件面板预览，仍限制在已授权项目内的 1 MB 文本文件。
- 任务完成、异常、待回答的未读标记；后台系统通知不展示会话正文，可在设置关闭。对已识别的 @pi-lab/notify 自定义通知脚本避免重复通知；其他第三方通知插件需各自配置。
- 中断回复独立保存在 Pi Desk 的 `recovery/` 私有目录，只作恢复预览。每条最多 32 个文本/思考块、合计 250,000 字符，约 1.5 秒节流保存；不包含图片或工具调用。与当前分支的用户消息核对，Pi 已保存的同时间戳回复优先；不回写 Pi 会话、不自动重发。
...
- 图片附件；运行中的追加输入可选择“插话”或“完成后发送”。队列按类型查看和复制，可确认清空全部；Pi 暂未提供逐条撤回接口。停止会清空队列、取消问答并等待 abort。
...
| Pi 返回的模型能力与已确认思考设置 | `userData/model-data/`，按配置和项目隔离，仅保存白名单元数据 |

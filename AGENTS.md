...
- Do not keep installed-app backups. Retain the previous app only during replacement for failure recovery, then delete it after successful installation. Keep build output under `release/build.noindex/` and remove unpacked app copies after successful release or Git installation, so Spotlight does not collect duplicate apps. Never delete Pi configuration or sessions during this cleanup.

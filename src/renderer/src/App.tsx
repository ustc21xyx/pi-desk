...
export default function App() {
  ...
  return <div className={`app-shell ${sidebar ? '' : 'sidebar-hidden'} ${visibleDiff || (panel && cwd) ? 'with-panel' : ''}`}>
    ...
    {visibleDiff ? <EditDiffPanel key={`${draftKey}:${visibleDiff.id}`} path={visibleDiff.path} diff={visibleDiff.diff} onClose={() => setSelectedDiff(undefined)} onFiles={cwd ? () => { setSelectedDiff(undefined); setPanel(true) } : undefined} /> : panel && cwd && <FilePanel key={cwd} cwd={cwd} onClose={() => setPanel(false)} />}

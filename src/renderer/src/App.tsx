...
import type { Bootstrap, DisplayMessage, ModelCatalog, ModelInfo, Preferences, RuntimeAction, RuntimeSnapshot, SessionInfo, RevisionDraft, EditDiff, RecoveryPreview } from '../../shared/contracts'
import { clean, ContextMeter, errorText, FilePanel, Mark, Modal, nameOf, phaseLabels, Question, Transcript } from './components'
import { FileReferenceInput } from './FileReferences'
import { ProjectPicker } from './ProjectPicker'
import { QueuePreview, RecoveredReply } from './SessionExtras'
import { RuntimeStatus } from './RuntimeStatus'
import { UpdateSection } from './UpdateSection'
import { EditDiffPanel } from './EditDiffPanel'
import { AnimatedPanel } from './AnimatedPanel'
import { SidebarResize, useSidebarWidths } from './SidebarResize'
...
const messageKey = (m: DisplayMessage) => m.timestamp ? `${m.role}:${m.timestamp}:${m.toolCallId || ''}` : m.id
export default function App() {
  ...
  const [earlier, setEarlier] = useState<{ key: string; rows: DisplayMessage[]; hasMore: boolean }>({ key: '', rows: [], hasMore: false }), [loadingOlder, setLoadingOlder] = useState(false), [historyMore, setHistoryMore] = useState(false)
  const [recovered, setRecovered] = useState<RecoveryPreview>(), [filePreview, setFilePreview] = useState<{ scope: string; path: string; version: number }>()
  const [sendBehavior, setSendBehavior] = useState<'steer' | 'followUp'>('steer')
  ...
  const [selectedDiff, setSelectedDiff] = useState<{ scope: string; id: string; path: string; diff: EditDiff }>()
  const [projectLimits, setProjectLimits] = useState<Record<string, number>>({})
  const [collapsedProjects, setCollapsedProjects] = useState<Record<string, boolean>>({})
  const [modal, setModal] = useState<'settings' | 'models' | 'catalog' | 'thinking' | 'draft-thinking' | 'commands' | 'rename' | null>(null), [modelQuery, setModelQuery] = useState(''), [rename, setRename] = useState('')
  ...
  const baseMessages = active.kind === 'history' ? history : runtime?.messages
  const messages = useMemo(() => { const base = baseMessages || []; const keys = new Set(base.map(messageKey)); return [...(earlier.key === draftKey ? earlier.rows.filter(m => !keys.has(messageKey(m))) : []), ...base] }, [baseMessages, earlier, draftKey])
  const hasEarlier = earlier.key === draftKey ? earlier.hasMore : active.kind === 'history' ? historyMore : (runtime?.messages.length || 0) >= 500
  ...
  useEffect(() => { pendingSettings.current = {}; setDraftModel(undefined); setDraftThinking(undefined); setProbedThinking(undefined); setCatalog({ models: [], source: 'recent' }); setSettingsId('') }, [boot?.preferences.agentDir, boot?.preferences.executable, boot?.preferences.node])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setHistory([]); setHistoryLoading(true); setHistoryNotice(''); setHistoryMore(false)
    void window.desk.history(active.path).then(result => { if (generation.current === id) { setHistory(result.messages); setHistoryMore(result.hasMore); setHistoryNotice(result.notice || '') } }).catch(e => { if (generation.current === id) notify(e) }).finally(() => { if (generation.current === id) setHistoryLoading(false) })
    return () => { generation.current++ }
  }, [active])
  useEffect(() => { setEarlier({ key: '', rows: [], hasMore: false }); setLoadingOlder(false) }, [draftKey])
  useEffect(() => { void window.desk.viewing(active.kind === 'runtime' ? active.id : active.kind === 'history' ? Object.values(latest.current).find(r => r.sessionPath === active.path)?.id || null : null).catch(() => {}) }, [active])
  const recoveryPath = runtime?.sessionPath || selectedHistory?.path
  useEffect(() => {
    let current = true; setRecovered(undefined)
    if (recoveryPath && (!runtime || !runtime.prepared && !runtime.settingsOnly && ['idle', 'error', 'closed'].includes(runtime.phase))) void window.desk.recovery(recoveryPath).then(value => { if (current) setRecovered(value) }).catch(() => {})
    return () => { current = false }
  }, [recoveryPath, runtime?.phase, runtime?.completedRuns])
  async function loadOlder(beforeCommit: () => void) {
    const path = runtime?.sessionPath || selectedHistory?.path, before = messages[0]?.entryId, key = draftKey, version = generation.current
    if (!path || !before || loadingOlder) return
    setLoadingOlder(true)
    try {
      const page = await window.desk.history(path, before)
      if (generation.current === version) { beforeCommit(); setEarlier(old => ({ key, rows: [...page.messages, ...(old.key === key ? old.rows : [])], hasMore: page.hasMore })) }
    } catch (e) { if (generation.current === version) notify(e) }
    finally { if (generation.current === version) setLoadingOlder(false) }
  }
  const showFile = useCallback((path: string) => { setFilePreview(old => ({ scope: draftKey, path, version: (old?.version || 0) + 1 })); setSelectedDiff(undefined); setPanel(true) }, [draftKey])
  useEffect(() => {
  ...
  async function connect(): Promise<string | null> {
    ...
    try {
      ...
      if (!path && active.kind === 'new') {
        const selected = pendingSettings.current, actual = latest.current[id]?.model
        if (selected.model && (actual?.id !== selected.model.modelId || actual?.provider !== selected.model.provider)) await window.desk.action(id, { type: 'model', ...selected.model })
  ...
  async function openAgentSettings(kind: 'models' | 'thinking') {
    if (busy || sending || savingSettings) return
    setError(''); setModelQuery('')
    if (active.kind === 'new') {
    ...
    setLoadingSettings(kind)
    try { await connect() } catch (e) { notify(e) } finally { setLoadingSettings(null) }
  }
  async function readDraftThinking() {
    if (!displayedModel || probingThinking) return
    const model = displayedModel, key = draftThinkingKey
    setProbingThinking(true); setError('')
    try {
      const id = await window.desk.prepareSettings()
      if (!project) setSettingsId(id)
      await waitUntilReady(id)
      const before = latest.current[id]?.model
      if (before?.id !== model.id || before?.provider !== model.provider) await window.desk.action(id, { type: 'model', provider: model.provider, modelId: model.id })
      const actual = latest.current[id]
      if (actual?.model?.id !== model.id || actual.model.provider !== model.provider || actual.settingsErrors?.thinking) throw new Error('暂时未能读取这个模型的档位，请稍后重试。')
      setProbedThinking({ key, levels: actual.thinkingLevels })
    } catch (e) { notify(e) } finally { setProbingThinking(false) }
  }
  function chooseDraftThinking(level?: string) {
    if (level && !displayedModel) return
    if (level && displayedModel) pendingSettings.current = { model: { provider: displayedModel.provider, modelId: displayedModel.id }, thinking: level }
    else { const { thinking: _thinking, ...selection } = pendingSettings.current; pendingSettings.current = selection }
    if (level && draftCapabilityModel) setDraftModel(draftCapabilityModel)
    setDraftThinking(level); setModal(null)
  }
  async function applyAgentSetting(action: RuntimeAction, close = true) {
    ...
    try {
      ...
      if (runtime.settingsOnly || runtime.prepared) {
        if (action.type === 'model') { pendingSettings.current = { model: { provider: action.provider, modelId: action.modelId } }; setDraftModel(runtime.models.find(m => m.provider === action.provider && m.id === action.modelId)); setDraftThinking(undefined) }
        if (action.type === 'thinking' && runtime.model) { pendingSettings.current = { model: { provider: runtime.model.provider, modelId: runtime.model.id }, thinking: action.level }; setDraftThinking(action.level) }
      }
  ...
  async function send() {
    ...
    try {
      ...
      await window.desk.action(id, { type: 'prompt', message, images: selectedImages.map(({ name: _name, ...image }) => image), behavior: latestRuntime && running(latestRuntime.phase) ? sendBehavior : undefined })
  ...
  const sessionRow = (session: SessionInfo) => {
    ...
    return <div key={session.path} className={`session-row ${activePath === session.path ? 'selected' : ''}`}>
      <button className="session-main" onClick={() => openSession(session)} title={session.title}>{rt?.unread ? <span className={`unread-dot ${rt.unread}`} title={rt.unread === 'question' ? '等待回答' : rt.unread === 'error' ? '有未查看的异常' : '有新回复'} /> : rt && running(rt.phase) ? <span className={`status-dot ${rt.phase}`} /> : preferences?.pinned.includes(session.path) ? <Pin size={12} /> : <MessageSquare size={13} />}<span>{session.title}</span><time>{relativeTime(session.updatedAt)}</time></button>
  ...
  return <div className={`app-shell ${sidebar ? '' : 'sidebar-hidden'} ${panelVisible ? 'with-panel' : ''} ${sidebarWidths.dragging ? 'resizing-sidebars' : ''}`} style={sidebarWidths.style}>
    {sidebar && <aside className="sidebar"><div className="window-drag sidebar-drag" /><div className="sidebar-brand"><Mark /><span>Pi Desk</span><button className="icon-button" title="收起侧栏 ⌘B" onClick={() => setSidebar(false)} aria-label="收起侧栏"><PanelLeftClose size={17} /></button></div>
      ...
      <div className="sidebar-scroll">
        ...
        {pendingRuntimes.map(r => <button key={r.id} className={`runtime-row ${active.kind === 'runtime' && active.id === r.id ? 'selected' : ''}`} onClick={() => { setActive({ kind: 'runtime', id: r.id }); setProject(r.cwd) }}><span className={r.unread ? `unread-dot ${r.unread}` : `status-dot ${r.phase}`} /><span>{r.title}<small>{nameOf(r.cwd)}</small></span><small>{phaseLabels[r.phase]}</small></button>)}
    ...
    <main className="workspace">{sidebarWidths.leftVisible && <SidebarResize side="left" width={sidebarWidths.left} min={200} max={sidebarWidths.leftMax} onChange={value => sidebarWidths.change('left', value)} onDragging={sidebarWidths.setDragging} />}{panelVisible && <SidebarResize side="right" width={sidebarWidths.right} min={300} max={sidebarWidths.rightMax} onChange={value => sidebarWidths.change('right', value)} onDragging={sidebarWidths.setDragging} />}<header className="topbar window-drag">{!sidebar && <button className="icon-button" onClick={() => setSidebar(true)} title="展开侧栏" aria-label="展开侧栏"><PanelLeftOpen size={18} /></button>}<div className="breadcrumb"><span>{cwd ? nameOf(cwd) : '工作区'}</span><ChevronRight size={13} /><strong>{viewTitle}</strong></div><div className="topbar-actions"><div className={`connection-state ${runtime?.phase || ''}`}><span className="status-dot" />{runtime && !runtime.prepared && !runtime.settingsOnly ? <RuntimeStatus runtime={runtime} /> : runtimeStatus}</div><button className={`icon-button ${panel || visibleDiff ? 'active' : ''}`} disabled={!cwd} title="文件与修改差异" aria-label="文件与修改差异" onClick={() => { setSelectedDiff(undefined); setPanel(visibleDiff ? true : !panel) }}><PanelRight size={18} /></button><div className="menu-anchor"><button className="icon-button" disabled={(!runtime || runtime.settingsOnly || runtime.prepared) && !isHistory} onClick={() => setMenu(v => !v)} title="会话操作" aria-label="会话操作"><MoreHorizontal size={19} /></button>{menu && <div className="popover session-menu">{activePath && <button onClick={() => { setRename(viewTitle); setModal('rename'); setMenu(false) }}>重命名会话</button>}{runtime && <><button onClick={() => { void perform({ type: 'refresh' }); setMenu(false) }}>刷新模型与状态</button><button disabled={busy} onClick={() => { void perform({ type: 'compact' }); setMenu(false) }}>压缩上下文</button><button onClick={() => { void perform({ type: 'close' }); setMenu(false) }}>结束此连接</button></>}{activePath && <><button onClick={() => { void toggleMeta('pinned', activePath); setMenu(false) }}>{preferences?.pinned.includes(activePath) ? '取消置顶' : '置顶会话'}</button><button onClick={() => { void toggleMeta('archived', activePath); setMenu(false) }}>{preferences?.archived.includes(activePath) ? '取消归档' : '归档会话'}</button></>}</div>}</div></div></header>
      ...
      {historyLoading ? <div className="center-state"><LoaderCircle className="spin" size={23} /><span>正在打开会话…</span></div> : messages.length ? <Transcript key={active.kind === 'runtime' ? active.id : active.kind === 'history' ? active.path : 'new'} messages={messages} runtime={runtime} hasMore={hasEarlier && !!messages[0]?.entryId} loadOlder={loadOlder} loadingOlder={loadingOlder} onFile={showFile} selectedDiffId={visibleDiff?.id} onShowDiff={value => { setSelectedDiff({ ...value, scope: draftKey }); setPanel(false) }} onRevise={(message, mode) => void beginRevision(message, mode)} revisionDisabled={controlsLocked || revisionLoading || revisionSending || runtime?.phase === "starting"} /> : <div className="welcome"><div className="welcome-content"><div className="welcome-overline"><Mark /><span>PI DESK</span></div><h1>从一个想法开始。</h1><p>与本机 Pi 一起，读懂代码，解决问题，创造新东西。</p><div className="welcome-project">{cwd ? <><FolderOpen size={16} /><span title={cwd}>{nameOf(cwd)}</span><span className="small-dot" />本地项目</> : <button onClick={() => void chooseProject()}><FolderOpen size={16} />选择项目文件夹<ChevronRight size={15} /></button>}</div><div className="suggestion-grid">{suggestions.map(s => <button key={s.title} onClick={() => { updateDraft(s.prompt); textRef.current?.focus() }}><s.icon size={18} /><strong>{s.title}</strong><span>{s.detail}</span></button>)}</div>{runtime?.phase === 'starting' && !runtime.settingsOnly && !runtime.prepared && <div className="starting-note"><LoaderCircle className="spin" size={15} />正在加载本机 Pi 与扩展…</div>}</div></div>}
      <div className="composer-region">
        ...
        {recovered && recoveryPath && <RecoveredReply value={recovered} error={notify} dismiss={async () => { await window.desk.dismissRecovery(recoveryPath); setRecovered(undefined) }} />}
        {runtime && <QueuePreview runtime={runtime} clear={() => window.desk.action(runtime.id, { type: 'clearQueue' })} error={notify} />}
        ...
        <div className="composer-box">
          ...
          <FileReferenceInput key={draftKey} cwd={cwd} value={draft} onChange={updateDraft} inputRef={textRef} onSend={() => void send()} />
    ...
    <AnimatedPanel>{visibleDiff ? <EditDiffPanel key={`${draftKey}:${visibleDiff.id}`} path={visibleDiff.path} diff={visibleDiff.diff} onClose={() => setSelectedDiff(undefined)} onFiles={cwd ? () => { setSelectedDiff(undefined); setPanel(true) } : undefined} /> : panel && cwd ? <FilePanel key={`${cwd}:${filePreview?.scope === draftKey ? filePreview.version : ''}`} initialPath={filePreview?.scope === draftKey ? filePreview.path : undefined} cwd={cwd} onClose={() => setPanel(false)} /> : null}</AnimatedPanel>
    ...
    {modal === 'thinking' && runtime && runtime.phase !== 'starting' && <Modal title="思考强度" onClose={() => setModal(null)}>
      ...
      {runtime.settingsErrors?.thinking ? <div className="picker-empty"><p>{runtime.settingsErrors.thinking}</p><button className="secondary-button" disabled={savingSettings || busy} onClick={() => void applyAgentSetting({ type: 'refresh' }, false)}>重新加载</button></div> : <div className="picker-list thinking-options">
        {gatewayAvailable && <button disabled={savingSettings || busy} onClick={() => void applyAgentSetting({ type: 'thinking', level: 'default' })}><span><strong>上游默认</strong><small>由服务商决定推理方式</small></span>{upstreamDefault && <Check size={15} />}</button>}
...
function SettingsModal({ boot, save, refresh, onClose, onError }: { boot: Bootstrap; save: (patch: Partial<Preferences>) => Promise<void>; refresh: () => Promise<void>; onClose: () => void; onError: (e: unknown) => void }) {
  ...
  const settingsDirty = JSON.stringify(naming) !== JSON.stringify(boot.preferences.naming) || (Object.keys(values) as (keyof typeof values)[]).some(key => values[key] !== boot.preferences[key])
  return <Modal title="设置" wide onClose={onClose}><div className="settings-body"><div className="settings-intro"><Mark /><div><h3>你的 Pi，新的工作界面。</h3><p>Pi Desk 使用这台 Mac 上的 Pi 和现有配置。</p></div><span className="version-pill">{boot.version}</span></div>
    <UpdateSection settingsDirty={settingsDirty} />
    <div className="setting-section"><h4>任务通知</h4><label className="setting-description"><input type="checkbox" checked={boot.preferences.desktopNotifications} onChange={e => void save({ desktopNotifications: e.target.checked }).catch(onError)} /> 在后台完成任务或需要回答时通知</label><p className="setting-description">正在查看的会话保持安静；侧栏标记未读结果。识别到 @pi-lab/notify 的自定义通知脚本时，不重复发送系统通知。</p></div>

...
import type { Bootstrap, DisplayMessage, ModelCatalog, ModelInfo, Preferences, RuntimeAction, RuntimeSnapshot, SessionInfo, RevisionDraft, EditDiff } from '../../shared/contracts'
...
export default function App() {
  ...
  return <div className={`app-shell ${sidebar ? '' : 'sidebar-hidden'} ${panelVisible ? 'with-panel' : ''} ${sidebarWidths.dragging ? 'resizing-sidebars' : ''}`} style={sidebarWidths.style}>
    ...
    <main className="workspace">{sidebarWidths.leftVisible && <SidebarResize side="left" width={sidebarWidths.left} min={200} max={sidebarWidths.leftMax} onChange={value => sidebarWidths.change('left', value)} onDragging={sidebarWidths.setDragging} />}{panelVisible && <SidebarResize side="right" width={sidebarWidths.right} min={300} max={sidebarWidths.rightMax} onChange={value => sidebarWidths.change('right', value)} onDragging={sidebarWidths.setDragging} />}<header className="topbar window-drag">{!sidebar && <button className="icon-button" onClick={() => setSidebar(true)} title="展开侧栏" aria-label="展开侧栏"><PanelLeftOpen size={18} /></button>}<div className="breadcrumb"><span>{cwd ? nameOf(cwd) : '工作区'}</span><ChevronRight size={13} /><strong>{viewTitle}</strong></div><div className="topbar-actions"><span className={`connection-state ${runtime?.phase || ''}`}><span className="status-dot" />{runtimeStatus}</span><button className={`icon-button ${panel || visibleDiff ? 'active' : ''}`} disabled={!cwd} title="文件与修改差异" aria-label="文件与修改差异" onClick={() => { setSelectedDiff(undefined); setPanel(visibleDiff ? true : !panel) }}><PanelRight size={18} /></button><div className="menu-anchor"><button className="icon-button" disabled={(!runtime || runtime.settingsOnly || runtime.prepared) && !isHistory} onClick={() => setMenu(v => !v)} title="会话操作" aria-label="会话操作"><MoreHorizontal size={19} /></button>{menu && <div className="popover session-menu">{activePath && <button onClick={() => { setRename(viewTitle); setModal('rename'); setMenu(false) }}>重命名会话</button>}{runtime && <><button onClick={() => { void perform({ type: 'refresh' }); setMenu(false) }}>刷新模型与状态</button><button disabled={busy} onClick={() => { void perform({ type: 'compact' }); setMenu(false) }}>压缩上下文</button><button onClick={() => { void perform({ type: 'close' }); setMenu(false) }}>结束此连接</button></>}{activePath && <><button onClick={() => { void toggleMeta('pinned', activePath); setMenu(false) }}>{preferences?.pinned.includes(activePath) ? '取消置顶' : '置顶会话'}</button><button onClick={() => { void toggleMeta('archived', activePath); setMenu(false) }}>{preferences?.archived.includes(activePath) ? '取消归档' : '归档会话'}</button></>}</div>}</div></div></header>
      ...
      {historyLoading ? <div className="center-state"><LoaderCircle className="spin" size={23} /><span>正在打开会话…</span></div> : messages.length ? <Transcript key={active.kind === 'runtime' ? active.id : active.kind === 'history' ? active.path : 'new'} messages={messages} runtime={runtime} selectedDiffId={visibleDiff?.id} onShowDiff={value => { setSelectedDiff({ ...value, scope: draftKey }); setPanel(false) }} onRevise={(message, mode) => void beginRevision(message, mode)} revisionDisabled={controlsLocked || revisionLoading || revisionSending || runtime?.phase === "starting"} /> : <div className="welcome"><div className="welcome-content"><div className="welcome-overline"><Mark /><span>PI DESK</span></div><h1>从一个想法开始。</h1><p>与本机 Pi 一起，读懂代码，解决问题，创造新东西。</p><div className="welcome-project">{cwd ? <><FolderOpen size={16} /><span title={cwd}>{nameOf(cwd)}</span><span className="small-dot" />本地项目</> : <button onClick={() => void chooseProject()}><FolderOpen size={16} />选择项目文件夹<ChevronRight size={15} /></button>}</div><div className="suggestion-grid">{suggestions.map(s => <button key={s.title} onClick={() => { updateDraft(s.prompt); textRef.current?.focus() }}><s.icon size={18} /><strong>{s.title}</strong><span>{s.detail}</span></button>)}</div>{runtime?.phase === 'starting' && !runtime.settingsOnly && !runtime.prepared && <div className="starting-note"><LoaderCircle className="spin" size={15} />正在加载本机 Pi 与扩展…</div>}</div></div>}
    ...
    <AnimatedPanel>{visibleDiff ? <EditDiffPanel key={`${draftKey}:${visibleDiff.id}`} path={visibleDiff.path} diff={visibleDiff.diff} onClose={() => setSelectedDiff(undefined)} onFiles={cwd ? () => { setSelectedDiff(undefined); setPanel(true) } : undefined} /> : panel && cwd ? <FilePanel key={cwd} cwd={cwd} onClose={() => setPanel(false)} /> : null}</AnimatedPanel>

...
// Public, fixed repository only. No Pi config, authorization headers or browser cookies.
async function request(url: string, signal: AbortSignal): Promise<Response> {
  for (let redirects = 0; redirects < 5; redirects++) {
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      ...
      if (!next) throw new Error('更新下载地址无效。')
...
export class AppUpdates {
  ...
  async check() {
    ...
    try {
      ...
      if (response.status === 403 || response.status === 429) throw new Error('GitHub 暂时限制了更新检查，请稍后重试。')
      if (!response.ok || !response.body) throw new Error('无法读取更新信息，请稍后重试。')
      const chunks: Uint8Array[] = []; let size = 0
      for await (const chunk of response.body as any as AsyncIterable<Uint8Array>) { size += chunk.length; if (size > 1024 * 1024) throw new Error('更新信息过大。'); chunks.push(chunk) }
      ...
      if (!match || release.draft || release.prerelease) throw new Error('更新版本信息无效。')
      ...
      if (!asset || asset.browser_download_url !== expectedURL || !Number.isSafeInteger(asset.size) || asset.size <= 0 || asset.size > maxDownload || !/^sha256:[a-f0-9]{64}$/.test(asset.digest || '')) throw new Error('此版本尚无适配这台 Mac 的完整更新包，请稍后再检查。')
  ...
  async stage() {
    ...
    if (!allowed.includes(target) || await fs.realpath(target) !== target) throw new Error('请先将 Pi Desk 放入“应用程序”文件夹，再使用应用内更新。')
    ...
    let mounted = false
    try {
      if (await hashFile(this.download) !== this.asset.digest) throw new Error('安装包已发生变化，请重新下载。')
      ...
      if (!(await fs.lstat(source)).isDirectory()) throw new Error('安装包中没有有效的 Pi Desk。')
  ...
  async handoff(prepared: Awaited<ReturnType<AppUpdates['stage']>>) {
    ...
    throw new Error('安装助手未能启动，当前应用不会退出。')
  }
